...
python3 -u "$ROOT_DIR/utils/http-check.py" "$DOMAINS_FILE"
echo

# echo "[3/13] Internal Signature Matching..."
# python3 -u "$ROOT_DIR/modules/matcher.py" "$DOMAINS_FILE"
# echo

echo "[3/12] Can-I-Take-Over-Xyz Fingerprint Matching..."
python3 -u "$ROOT_DIR/3rd/can-i-take-over-xyz/can-i-take-over-xyz.py" "$DOMAINS_FILE"
echo

echo
echo "[4/12] SubJack Fingerprint Matching..."
python3 -u "$ROOT_DIR/3rd/subjack/subjack.py" "$DOMAINS_FILE"
echo

echo
echo "[5/12] SubOver Fingerprint Matching..."
python3 -u "$ROOT_DIR/3rd/subover/subover.py" "$DOMAINS_FILE"
echo

echo
echo "[6/12] Tko-Subs Fingerprint Matching..."
...
python3 -u "$ROOT_DIR/modules/report.py" "$DOMAINS_FILE"
echo
