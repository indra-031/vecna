...
OUTPUT_FILE = os.path.join(FOUND_DIR, "subjack.json")
...
def match_subjack(domain, dns_data, http_data, fingerprints):
    ...
    is_nxdomain = domain_dns.get("NXDOMAIN", False)

    for fp in fingerprints:

        service = fp.get("service", "unknown")
        cname_patterns = [c.lower() for c in ensure_list(fp.get("cname"))]
        fingerprints_patterns = ensure_list(fp.get("fingerprint"))
        requires_nxdomain = fp.get("nxdomain", False)
        ...
        for pattern in fingerprints_patterns:
...
def main():
    ...
    if len(sys.argv) != 2:
        print("Usage: python subjack.py domains.txt")
        sys.exit(1)
    ...
    for domain in targets:
        ...
        findings = match_subjack(domain, dns_data, http_data, fingerprints)
    ...
    if all_findings:
        ...
        print(f"[+] Found {len(all_findings)} vulnerable target(s) via Subjack.")
    else:
        print("[-] No Subjack matches found.")
